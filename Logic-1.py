...
def cigar_party(cigars, is_weekend):
    if is_weekend == True:
      if cigars >= 40:
        ...
        return False
    return True if cigars in range(40, 61) else False


# Logic-1 > date_fashion
"""""
You and your date are trying to get a table at a restaurant. The parameter "you" is the stylishness of
your clothes, in the range 0..10, and "date" is the stylishness of your date's clothes. The result getting
the table is encoded as an int value with 0=no, 1=maybe, 2=yes. If either of you is very stylish, 8 or 
more, then the result is 2 (yes). With the exception that if either of you has style of 2 or less, then the 
...
def alarm_clock(day, vacation):
  if day in range(1, 6) and vacation:
    return '10:00'
  if day not in range(1, 6) and vacation:
    return 'off'
  if day in range(1, 6) and not vacation:
    return '7:00'
  if day not in range(1, 6) and not vacation:
    return '10:00'
...
def love6(a, b):
  if a == 6 or b == 6:
    return True
  return True if (a + b) == 6 or abs(a - b) == 6 else False
...
"""""
def near_ten(num):
  if num % 10 == 1 or num % 10 == 2 or (10 - num % 10 == 1) or (10 - num % 10 == 2) or num % 10 == 0 :
    return True
  else:
    return False
